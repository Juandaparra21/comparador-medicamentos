    ...
    <section className="mx-auto px-4 sm:px-5 max-w-3xl py-8 sm:py-12 w-full">
      <header className="mb-6 grid grid-cols-1 sm:grid-cols-[1fr_200px] gap-4 items-center">
        <div>
          ...
          <p className="text-[14px] text-[#717786] mt-1.5 leading-relaxed">
            Farmacias a 2 km a la redonda, en tiempo real desde OpenStreetMap. Usa tu ubicación o
            escribe tu dirección. Los precios, cuando existen, vienen de las farmacias en línea que comparamos.
          </p>
        </div>
        <img
      ...
      <div className="glass-card rounded-2xl p-5 mb-5">
        ...
        >
          ...
          {status === 'locating' ? 'Ubicando...' : status === 'loading' ? 'Buscando farmacias...' : 'Usar mi ubicación'}
        </button>

        {/* Manual address entry with autocomplete (also used when geolocation is denied) */}
        <div className="mt-3 flex items-center gap-3">
          <div className="flex-1 h-px bg-[#e5e7eb]" />
          <span className="text-[11px] font-semibold text-[#9ca3af] uppercase tracking-wide">o ingresa tu dirección</span>
          <div className="flex-1 h-px bg-[#e5e7eb]" />
        </div>
        <div className="mt-3">
          <AddressAutocomplete
            onSelect={(s) => goTo(s.lat, s.lng)}
            placeholder="Dirección, barrio o ciudad (ej: Calle 53 # 25-10, Bogotá)"
            disabled={status === 'locating' || status === 'loading'}
          />
          <p className="text-[11px] text-[#9ca3af] mt-1.5">
            Escribe y elige una sugerencia. Luego puedes arrastrar el punto azul del mapa para afinar tu dirección.
          </p>
        </div>
      ...
      {/* Foto ilustrativa mientras el usuario aún no ha buscado */}
      {status === 'idle' && (
      ...
      {status === 'loading' && (
        <div className="flex justify-center py-20">
          <BrandLoader label="Consultando farmacias cercanas..." />
        </div>
      )}
      ...
      {status === 'ready' && pharmacies.length > 0 && (
        <>
            ...
            <div className="rounded-2xl overflow-hidden border border-white/60 shadow-sm mb-4">
              <div className="flex items-center justify-between gap-3 px-4 py-2 bg-white/70 border-b border-[#f0f1f5] text-[11px] text-[#717786]">
                <span className="flex items-center gap-1 font-semibold text-primary">
                  ...
                  Arrastra el punto azul para fijar tu dirección
                </span>
          ...
          <p className="text-[11px] text-[#c1c6d7] mt-5 leading-relaxed">
            Datos de ubicación: OpenStreetMap (colaboradores de OSM). &ldquo;Con precios&rdquo; son sedes de las
            cadenas cuyos precios comparamos. Farmi es independiente y no está afiliado a estas farmacias; sus marcas
      ...
      <div className="mt-3 pt-3 border-t border-[#f0f1f5]">
        {medSubmitted ? (
          ...
          <p className="text-[12px] text-[#717786]">
            Comparamos precios en línea de {p.chainName}. Escribe un medicamento arriba para verlos.
          </p>
        ) : (
          <p className="text-[12px] text-[#9ca3af]">Sin datos de precios para esta farmacia</p>
        )}
